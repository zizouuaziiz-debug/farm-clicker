...
function PlotCard({
  ...
  const targetRef = useRef(plot.growthPercent || 0);
  const currentRef = useRef(plot.growthPercent || 0);

  useEffect(() => {
    targetRef.current = plot.growthPercent || 0;
  }, [plot.growthPercent]);
  ...
  useEffect(() => {
    const animate = () => {
      ...
      const newValue = current + diff * 0.08;
  ...
  if (plot.readyAt && plot.state === "growing") {
    secondsLeft.current = Math.max(
      0,
      Math.floor((new Date(plot.readyAt).getTime() - Date.now()) / 1000)
    );
  }

  const circleOffset = 283 - (283 * smoothProgress) / 100;
        ...
        return (
          <div className="relative w-full h-full flex flex-col items-center justify-center">
            <motion.span
              ...
              className="text-4xl z-10 drop-shadow-[0_8px_6px_rgba(0,0,0,.5)]"
            >
              {getCropStage(plot.cropType, smoothProgress)}
            </motion.span>

            <div className="bg-black/60 text-white text-[10px] px-2 py-1 rounded-full z-10">
              {formatTime(secondsLeft.current)}
            </div>
        ...
        return (
          <div className="relative">
            <span className="text-4xl opacity-80 drop-shadow-lg">
              {getCropStage(plot.cropType, plot.growthPercent || 50)}
            </span>

            <div className="absolute -top-3 -right-3 bg-blue-500 rounded-full p-1 animate-bounce border-2 border-white">
              <Droplets size={16} />
            </div>
          </div>
        );
      ...
      case "ready":
        return (
          <motion.div
            ...
            className="relative"
          >
            <span className="text-5xl drop-shadow-[0_10px_8px_rgba(0,0,0,.6)]">
              {CROP_EMOJIS[plot.cropType!] || "🌾"}
            </span>

            <Sparkles
              size={22}
              className="absolute -top-3 -right-3 text-yellow-300 animate-spin"
              fill="currentColor"
            />
          </motion.div>
        );
      ...
      case "dead":
        return (
          <div className="flex flex-col items-center">
            ...
            <span className="bg-black/50 text-white text-[10px] rounded-full px-2 py-1">
              Clear
            </span>
          </div>
        );
  ...
  const backgrounds: Record<string, string> = {
    empty: "bg-[#8b5a2b]",
    growing: "bg-[#654321]",
    needs_water: "bg-[#806044]",
    ready: "bg-[#4a3728] border-2 border-yellow-400",
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      className={`
        relative w-full h-full rounded-2xl flex items-center justify-center overflow-hidden border border-white/10
        ${backgrounds[plot.state] || backgrounds.empty}
        shadow-[0_12px_20px_rgba(0,0,0,.45),inset_0_-12px_18px_rgba(0,0,0,.35),inset_0_5px_12px_rgba(255,255,255,.18)]
...
export default function Farm() {
  ...
  const handlePlotClick = useCallback(
    (plot: FarmPlot) => {
      ...
      if (plot.state === "withered" || plot.state === "dead") {
        harvestPlot.mutate(
          { plotId: plot.id },
          {
            onSuccess: () => {
    ...
    (cropType: string) => {
      ...
      plantSeed.mutate(
        ...
        },
        {
          ...
          onError: (err: any) => {
            toast.error(err?.message || "Failed to plant");
          },
        }
      );
    },
    [selectedSlot, isBusy, plantSeed, queryClient]
  );
  ...
  const availableSeeds = inventory.filter(
    (i) => i.itemType.endsWith("_seed") && i.quantity > 0
  );
      ...
      <AnimatePresence>
        {selectedSlot !== null && (
          <>
            ...
            >
              <div className="bg-card rounded-2xl border p-5 shadow-2xl">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="font-bold text-lg">Plant Seed</h2>
                  <button
                    onClick={() => setSelectedSlot(null)}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {availableSeeds.map((seed) => {
                      ...
                      return (
                        <div
                          key={seed.id}
                          className="bg-muted/30 rounded-xl p-3 text-center border"
                        >
                          <div className="text-3xl">
                            {CROP_EMOJIS[type] || "🌱"}
                          </div>
                          <p className="capitalize font-bold">{type}</p>
                          <p className="text-xs">x{seed.quantity}</p>
